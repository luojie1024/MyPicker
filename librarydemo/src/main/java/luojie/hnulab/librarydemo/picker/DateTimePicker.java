package luojie.hnulab.librarydemo.picker;
...
import luojie.hnulab.librarydemo.utli.DateUtils;
import luojie.hnulab.librarydemo.utli.LogUtils;
...
public class DateTimePicker extends WheelPicker {
    ...
    @IntDef(value = {NONE, YEAR_MONTH_DAY, YEAR_MONTH, MONTH_DAY,YEAR})
     ...
     * FIXME
     */
    public void setDateRangeStart(int startYear) {
        if (dateMode == NONE) {
            throw new IllegalArgumentException("Date mode invalid");
        }
...
//            this.startDay = startMonthOrDay;
     ...
     * FIXME
     */
    public void setDateRangeEnd(int endYear) {
        if (dateMode == NONE) {
            throw new IllegalArgumentException("Date mode invalid");
        }
        if (dateMode == YEAR_MONTH_DAY) {
...
//            this.endDay = endMonthOrDay;
    ...
    public void setSelectedItem(int yearOrMonth, int monthOrDay, int hour, int minute) {
        ...
        if (dateMode == MONTH_DAY) {
            ...
            LogUtils.verbose(this, "change year while set selected");
            changeMonthData(yearOrMonth);
            selectedYearIndex = findItemIndex(years, yearOrMonth);
        }
        if (timeMode != NONE) {
    ...
    protected View makeCenterView() {
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY|| dateMode != YEAR) && days.size() == 0) {
         ...
         * time：2017/7/1 19:25
         */
        if (dateMode != NONE && dateMode !=YEAR) {
    ...
    protected void onSubmit() {
        ...
        switch (dateMode) {
            ...
            case MONTH_DAY:
                ...
                break;
            case YEAR:
                ((OnYearTimePickListener) onDateTimePickListener).onDateTimePicked(year);
                break;
            case NONE:
...
//        selectedMonthIndex = preSelectMonthIndex == -1 ? 0 : preSelectMonthIndex;
     ...
     * time：2017/7/1 19:10
     */
    private void changeDayData(int selectedYear) {
        int maxDays = DateUtils.calculateDaysInMonth(selectedYear);
        days.clear();
        months.clear();
...
//        selectedMinute = DateUtils.fillZero(Calendar.getInstance().get(Calendar.MINUTE));
    ...
    public interface OnYearTimePickListener extends OnDateTimePickListener {
        void onDateTimePicked(String year);
    }
